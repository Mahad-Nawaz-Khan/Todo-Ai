...
function GoogleIconGrayscale({ className }: { className?: string }) {
  return (
...
function GoogleIconColor({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
...
function GoogleLogo({ className }: { className?: string }) {
  return (
    <span className={`relative inline-block ${className}`}>
      <GoogleIconGrayscale className="h-5 w-5 text-gray-400 transition-opacity duration-200 group-hover:opacity-0" />
      <GoogleIconColor className="absolute left-0 top-0 h-5 w-5 opacity-0 transition-opacity duration-200 group-hover:opacity-100" />
    </span>
  );
}
      ...
      <div className="mx-auto flex min-h-screen max-w-6xl items-center justify-center px-6 py-12">
        <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/5 p-6 shadow-lg">
          <div className="mb-6">
            ...
            <p className="mt-1 text-sm text-white/70">Sign up with email or use a social provider.</p>
          </div>
            ...
            {providers.map((provider) => (
              ...
              >
                {provider.id === "google" ? (
                  ...
                  <GitHubIcon className="h-5 w-5 text-gray-400 transition-colors duration-200 group-hover:text-white" />
                )}
                {provider.label}
              </a>
            ))}
